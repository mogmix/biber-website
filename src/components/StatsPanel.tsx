      ...
      <div className="space-y-1.5" role="list" aria-label="Sichtungen nach Bundesland">
        {sorted.map(({ code, name, count }) => (
          <div key={code} role="listitem" className="flex items-center gap-2 text-xs sm:text-sm">
            ...
            <div
              role="progressbar"
              aria-label={`${name}: ${count} ${count === 1 ? 'Sichtung' : 'Sichtungen'}`}
              aria-valuenow={count}
              aria-valuemin={0}
            ...
            <span className="w-6 sm:w-8 shrink-0 text-right tabular-nums text-gray-600 dark:text-gray-400" aria-hidden="true">{count}</span>
          </div>
        ))}
      </div>
